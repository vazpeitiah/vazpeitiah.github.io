---
name: Algebreb
tools: [React, Bootstrap, Python, Flask, Sympy, MongoDB]
image: /assets/img/ALGEBREB.png
description: Aplicación web generadora y evaluadora de ejercicios algebraicos.
---
  
# ALGEBREB
{% include elements/button.html link="https://github.com/vazpeitiah/algebreb" text="GitHub" style="primary" size="sm" %}
{% include elements/button.html link="https://algebreb.herokuapp.com/" text="Demo" style="success" size="sm" %}

---

Algebreb es una aplicación web capaz de generar ejercicios de álgebra junto con sus soluciones paso a paso. De manera que tanto profesores como alumnos puedan verse beneficiados con ella, al poder realizar material de evaluación de una forma práctica y rápida, y al contar con ejercicios que puedan ayudar a mejorar su desempeño en la asignatura respectivamente.


{% capture carousel_images %}
https://i.imgur.com/uZso9HJ.png
https://i.imgur.com/WsHI6Tl.png
https://i.imgur.com/AiWiXPU.png
https://i.imgur.com/8QmLmYn.png
{% endcapture %}
{% include elements/carousel.html %}

<!-- ![algebreb01.png](https://i.imgur.com/9u8x7SZ.png "Building Image") -->


### Características principales
- Generación de ejercicios de álgebra

- Generación de solución final

- Generación de solución paso a paso

- Evaluaciones en línea

- Exportar ejercicios a PDF, LaTeX o JPG.

### Temas del álgebra
- Polinomios

- Productos notables

- Factorización

- Fracciones algebraicas

- Ecuaciones lineales

- Sistema de ecuacioneas en tres variables

- Sistema de ecuacioneas en dos variables

- Ecuaciones cuadráticas

### ¿Hacia quién va dirigido?
Profesores y estudiantes de nivel medio superior.

### Colaboradores
- Ivan Aldavera > [GitHub](https://github.com/zarzamora01)
- Ivan Vargas > [GitHub](https://github.com/ivanvg98)